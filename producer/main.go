...
var (
	...
	messageCount = flag.Int("message-count", 8, "Number of messages to publish")
...
func publish(publishOkCh <-chan struct{}, confirmsCh chan<- *amqp.DeferredConfirmation, confirmsDoneCh <-chan struct{}, exitCh chan struct{}) {
	...
	Log.Println("producer: sent all messages, waiting for confirms")
	close(exitCh)
	select {
	case <-confirmsDoneCh:
		Log.Println("producer: all confirms received, exiting")
